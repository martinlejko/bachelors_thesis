...
*   **Run Evaluation Tests:**
    ...
    There is a potential that you run into a problem and tests will fail due to incorrect JSON.
    ```
    ValueError: Evaluation LLM outputted an invalid JSON. Please use a better evaluation model.
    ```
    This is due to the fact that we are using a smaller local model and it is not able to generate a valid JSON. That is why we have implemented a retry policy that if we run into this error we try to re-run the tests.
