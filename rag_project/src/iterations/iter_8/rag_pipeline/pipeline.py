...
from src.common.models import Document, QueryResult, RetrievedDocument
...
class RagPipeline:
    ...
    def query(self, query: str) -> QueryResult:
        ...
        return QueryResult(query=query, answer=result["answer"], retrieved_documents=result["retrieved_documents"])
        ...
        def invoke_with_retrieval_context(question):
            ...
            for i, doc in enumerate(retrieved_docs):
                retrieved_documents.append(
                    RetrievedDocument(content=doc.page_content, metadata=doc.metadata, score=getattr(doc, "score", 0.0))
                )
